...
# MCP Tool Catalog
# ================================================================================
# Tool descriptions are written for AI/LLM consumption: clear usage, connection
# names, and examples so Cursor and other MCP clients can discover and use
# tools correctly across different instances.
# ================================================================================

def _server_instructions() -> str:
    """Build instructions string for initialize/serverInfo so clients (e.g. Cursor) know how to use this server."""
    conns = sorted(CONNECTIONS.keys())
    conn_list = ", ".join(conns) if conns else "none (no CONN_* configured)"
    return (
        f"This MCP server exposes PostgreSQL/Supabase database access. "
        f"Available connections: {conn_list}. "
        "Tools: (1) postgres.list_connections — call with no arguments to get connection names. "
        "(2) postgres.get_schema — pass connection (required) and optionally table name to list tables or columns. "
        "(3) postgres.query — pass connection and sql (required); optionally params array for $1,$2. "
        "Always use exact connection names from list_connections. "
        "Read-only by default; write operations require ALLOW_WRITE=true on the server."
    )
...
def tool_catalog() -> Dict[str, List[Dict[str, Any]]]:
    ...
    Descriptions are written so Cursor and other MCP clients can reliably
    discover tools and know how to call them (connection names, required args).
    """
    ...
    # Only add enum when we have connections; some clients misbehave with enum: []
    connection_schema: Dict[str, Any] = {
        "type": "string",
        "description": f"Database connection name. Must be one of: {connection_list}. Call postgres.list_connections to get the exact list.",
    }
    if available_connections:
        connection_schema["enum"] = available_connections
        ...
        "tools": [
            # Tool 1: List available database connections
            {
                ...
                "description": (
                    ...
                    "Call this first to get the exact connection names to use with postgres.get_schema and postgres.query. "
                    "Takes no arguments. Returns a list of {name, description} for each connection."
                ),
                "inputSchema": {
                    "type": "object",
                    "properties": {},
            ...
            # Tool 2: Get database schema information
            {
                ...
                "description": (
                    f"Get database schema for a connection. Connection must be one of: {connection_list}. "
                    "Required: connection (string). Optional: table (string). "
                    "If table is omitted, returns all tables in the public schema; if table is set, returns columns for that table."
                ),
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        ...
                        "table": {
                            "type": "string",
                            "description": "Optional. Table name to get columns for. Omit to list all tables in public schema.",
                        },
                    },
                    "required": ["connection"],
                    "additionalProperties": False,
            ...
            # Tool 3: Execute SQL query
            {
                "name": "postgres.query",
                "description": (
                    f"Execute a SQL query on a database. Connection must be one of: {connection_list}. "
                    "Required: connection (string), sql (string). Optional: params (array of strings for $1, $2 in SQL). "
                    "Only SELECT is allowed unless the server has ALLOW_WRITE=true. "
                    "Use postgres.list_connections to get valid connection names."
                ),
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        ...
                        "sql": {
                            "type": "string",
                            "description": "SQL query to execute, e.g. SELECT * FROM my_table LIMIT 10. Use $1, $2 for parameters when passing params.",
                        },
                        "params": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Optional. Parameters for parameterized queries; use $1, $2, ... in sql.",
                        },
                    },
...
async def api_list_tools(request: Request):
    """
    List MCP tools and how to use them (for debugging and Cursor setup).

    Returns tool names, descriptions, and parameter summaries so clients
    can verify the server exposes the expected tools.
    """
    ...
    for t in catalog.get("tools", []):
        ...
        params = [f"{k}{' (required)' if k in required else ''}: {props.get(k, {}).get('description', '')[:60]}..." for k in props]
...
async def mcp_endpoint(request: Request):
    ...
    # === INITIALIZE ===
    # Support both 2024-11-05 and 2025-03-26 for Cursor and other client compatibility
    if method in ("initialize", "server/initialize"):
        client_version = (params or {}).get("protocolVersion", "")
        protocol_version = "2025-03-26" if client_version == "2025-03-26" else "2024-11-05"
        result = {
