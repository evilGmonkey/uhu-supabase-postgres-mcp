...
3. You should see your server listed as **Connected**
4. It should show tools such as `postgres.list_connections`, `postgres.get_schema`, `postgres.query` (or `sql_query` depending on Cursor version)

**Check what the server exposes:** From a terminal, call the tools discovery endpoint (use your MCP URL and token):

```bash
curl -s -H "Authorization: Bearer YOUR_MCP_TOKEN" "http://localhost:8799/api/tools"
```

This returns tool names, descriptions, and connection names so you can confirm the server is configured correctly.
...
### Another Cursor instance fails to see or use the tools

1. **Use project-level config** so each workspace uses the right server: create `.cursor/mcp.json` in the project root with the same `url` and `Authorization` header as your working instance.
2. **Verify the server exposes tools:** Call `GET /api/tools` with your Bearer token (e.g. `curl -s -H "Authorization: Bearer YOUR_TOKEN" "http://your-server:8799/api/tools"`). You should see `postgres.list_connections`, `postgres.get_schema`, `postgres.query` and the `connection_names` list. If this fails, the problem is network/auth or server config, not Cursor.
3. **Reload the window** after changing MCP config: Command Palette → **Developer: Reload Window**.
4. **Match URL and token:** The `url` in `mcp.json` must reach the MCP server (e.g. `http://host:8799/mcp`), and the token must match `MCP_TOKEN` in the server’s `.env`.
